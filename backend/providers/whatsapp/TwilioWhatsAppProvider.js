...
function withWhatsappPrefix(number) {
  return String(number || "").startsWith("whatsapp:") ? String(number) : `whatsapp:${number}`;
}
...
module.exports = {
  ...
  validateConfig(rawConfig) {
    ...
    return {
      valid,
      errors: valid ? [] : ["Twilio WhatsApp requires account_sid, auth_token, and from_number."],
    };
  },
  async sendMessage(payload, rawConfig) {
    const config = normalizeConfig(rawConfig);
    const url = `https://api.twilio.com/2010-04-01/Accounts/${config.account_sid}/Messages.json`;
    const response = await requestForm(
      url,
      {
        From: withWhatsappPrefix(config.from_number),
        To: withWhatsappPrefix(payload.to),
        Body: payload.body,
