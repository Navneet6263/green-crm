...
BEGIN
  CREATE TABLE [dbo].[leads] (
    ...
    [is_workflow] BIT    NOT NULL DEFAULT 0,
    [workflow_status] NVARCHAR(50)   NULL,
    [completed_files] NVARCHAR(MAX)  NULL,
    [quality] NVARCHAR(30)   NULL,
    [expert_notes] NVARCHAR(MAX)   NULL,
    [expert_link] NVARCHAR(512)  NULL,
...
IF NOT EXISTS (
  SELECT 1
  FROM sys.indexes
  WHERE name = N'idx_ld_lead' AND object_id = OBJECT_ID(N'dbo.lead_documents')
)
BEGIN
...
IF NOT EXISTS (
  SELECT 1
  FROM sys.indexes
  WHERE name = N'idx_ld_company_lead_uploaded_perf' AND object_id = OBJECT_ID(N'dbo.lead_documents')
)
BEGIN
...
IF NOT EXISTS (
  SELECT 1
  FROM sys.indexes
  WHERE name = N'idx_ld_company_uploader_uploaded_perf' AND object_id = OBJECT_ID(N'dbo.lead_documents')
)
BEGIN
