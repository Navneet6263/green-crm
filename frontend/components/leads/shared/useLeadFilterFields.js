...
export function useLeadFilterFields({ companies, filterUsers, filters, leadOptionPool, productOptions, role, session, teams }) {
  ...
  return useMemo(
    ...
    [assigneeOptions, canManage, companies, createdByOptions, filters.assignedTo, filters.company, filters.createdBy, filters.hasFixedAssigneeScope, filters.isPlatformConsole, filters.priority, filters.product, filters.setAssignedTo, filters.setCompany, filters.setCreatedBy, filters.setPriority, filters.setProduct, filters.setSource, filters.setStatus, filters.setTeamFilter, filters.setWorkflowStage, filters.source, filters.status, filters.teamFilter, filters.workflowStage, products, sourceOptions, teams]
  );
}
