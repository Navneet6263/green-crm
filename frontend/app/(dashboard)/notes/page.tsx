...
export default function NotesPage() {
  ...
  const fetchNotes = useCallback(async () => {
    try {
      const fetched = await notesApi.list({
  ...
  const fetchTags = useCallback(async () => {
    try {
      const fetched = await notesApi.getTags();
      setTags(fetched);
    } catch {}
  }, []);
          ...
          <div className="text-center max-w-sm px-6">
            ...
            <p className="text-sm text-gray-500 mb-5">
              {notes.length === 0
                ...
                : 'Click any note on the left to open it here'}
            </p>
            {notes.length === 0 ? (
              <button
              ...
              <div className="space-y-2.5 text-xs text-gray-400 text-left bg-white border border-gray-100 rounded-xl p-4">
                <div className="flex items-center gap-2">
                  <Pin size={13} /> Pin important notes to top
                </div>
                <div className="flex items-center gap-2">
                  <Tag size={13} /> Use tags to organize
                </div>
                <div className="flex items-center gap-2">
                  <Sparkles size={13} /> Auto-saves as you type
                </div>
              </div>
            )}
          </div>
